...
class GPUHandler {
 public:
  ...
  // cross entropy loss
  static void cross_entropy(const float* x, const float* y,
                            float* z, std::vector<size_t> shape);
  // cross entropy loss backward for x
  static void cross_entropy_backward_x(const float* t_softmax, const float* y, float* x_grad,
